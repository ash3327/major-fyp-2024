...
from ..modules.gestures import extract_keypoints, extract_hand_keypoints
...
class ImageKptDataset(Dataset):
    ...
    def __getitem__(self, index):
        ...
        if isinstance(image_numpy, torch.Tensor):
            image_numpy = image_numpy.numpy().transpose((1, 2, 0))
        # print(image_numpy.shape, image_numpy.max())
        rimg, kpt, kpt_coords = extract_hand_keypoints(image_numpy)
        # print(kpt.shape)
        # target = torch.nn.functional.one_hot(, num_classes=self.get_unique_labels())
        return image, rimg, kpt, kpt_coords, label, self.unique_labels[label]
