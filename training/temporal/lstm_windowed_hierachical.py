...
eval_interval = 2
patience = 100
extra_text = f"Bi-directional LSTM, direct classification [{__file__}]"
...
# Model
# Model
model = LSTMGestureModel_Hierachical_Windowed(output_dim=num_classes, window_size=16, window_stride=window_stride).to(device)
model.to(device)
...
def evaluate_model():
    ...
    with torch.no_grad():
        ...
        for sequences, labels in progress_bar:
            ...
            outputs_flat = outputs.view(-1, model.output_dim)
            labels_flat = torch.argmax(labels_flat.view(-1,num_classes),dim=-1)
            mask = mask.view(-1)
