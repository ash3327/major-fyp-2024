...
class HandEncoder_6DOF(HandEncoder):
    def __init__(self, input_size=21 * 6, hidden_sizes=[256, 128, 64], embedding_size=128, dropout_rate=0.3, leaky_slope=0.01):
        """
        HandEncoder with 6DOF preprocessing and dynamically adjustable hidden layers, dropout, and modern enhancements.
        
        Args:
            input_size (int): Size of the flattened input (default: 21 joints * 6 coords = 126).
            hidden_sizes (list): List of integers specifying the sizes of hidden layers.
                                Default: [256, 128, 64] for a simpler, effective MLP.
            embedding_size (int): Size of the final embedding output (default: 128).
            dropout_rate (float): Dropout probability for hidden layers (default: 0.3).
            leaky_slope (float): Slope for LeakyReLU activation (default: 0.01).
        """
        super().__init__(input_size=input_size, hidden_sizes=hidden_sizes, embedding_size=embedding_size, 
                         dropout_rate=dropout_rate, leaky_slope=leaky_slope)
    ...
    def forward(self, x):
        ...
        return super().forward(x)  # Use the forward method from HandEncoder
