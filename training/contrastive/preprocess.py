...
def extract_orientations(joints, batched=False):
    ...
    try:
        ...
        null_entries_wrist = null_entries
        ...
        for finger in fingers:
            for j in range(len(finger) - 1):
                ...
                try:
                    null_entries = np.logical_or(null_entries_wrist,np.linalg.det(R_batch) == 0)
        ...
        print(f"ERROR",e)
        relative_orientations_batch = np.zeros((batch_size, 21, 4))
        # prevent training from stopping
