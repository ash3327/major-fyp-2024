...
class HandEncoderGAT3dof(nn.Module):
    def __init__(
        ...
        for i in range(num_layers):
            in_dim = current_dim if i == 0 else hidden_channels * num_heads
            concat = True # Keep concat=True for multi-head stability
            out_dim = hidden_channels

            conv = GATConv(in_channels=in_dim,
                           out_channels=out_dim,
                           heads=num_heads,
                           concat=concat,
                           dropout=dropout_rate,
                           # edge_dim=... # NO edge_dim here
                           add_self_loops=True)
            self.gat_layers.append(conv)

            bn_dim = out_dim * num_heads if concat else out_dim
            self.batch_norms.append(BatchNorm(bn_dim))
            current_dim = bn_dim
    ...
    def forward(self, data):
        ...
        for i in range(self.num_layers):
            x = self.gat_layers[i](x, edge_index) # No edge_attr needed
...
class HandEncoderGAT6dof(HandEncoderGAT3dof):
